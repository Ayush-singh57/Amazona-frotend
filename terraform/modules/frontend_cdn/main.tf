...
resource "aws_cloudfront_distribution" "cdn" {
  ...
  origin {
    ...
    custom_origin_config {
      http_port              = 80
      https_port             = 443
      origin_protocol_policy = "http-only" # Magic: CF talks to ALB securely over HTTP
      origin_ssl_protocols   = ["TLSv1.2"]
    }
  }

  # CUSTOM API ROUTING: Send anything with /api/* to the ALB
  ordered_cache_behavior {
    ...
    forwarded_values {
      query_string = true
      headers      = ["*"] # Crucial: passes Auth headers to your backend
      cookies {
        forward = "all"
      }
    }
